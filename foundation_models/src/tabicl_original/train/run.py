...
def negative_cosine_similarity(z1, z2):
...
def clr_transform(X, axis=-1, eps=1e-8):
    """
    Apply CLR transform to compositional data, safely handling zeros and negatives.
    ...
    # 1. Shift values to be positive per sample
    min_val = X.min(dim=axis, keepdim=True).values
    X_pos = X - min_val + eps

    # 2. Normalize to sum=1 along axis (composition)
    X_comp = X_pos / X_pos.sum(dim=axis, keepdim=True)

    # 3. Log transform
    logX = torch.log(X_comp)

    # 4. Subtract mean along axis for CLR
    clr_X = logX - logX.mean(dim=axis, keepdim=True)

    return clr_X

def closure(X):
    """
    Apply z-normalization to each sample independently to make them compositional.

    Parameters
    ----------
    X : torch.Tensor
        Input tensor of shape (B, T, H), where B=batch, T=sequence length, H=features.
    eps : float
    ...
    -------
    X_norm : torch.Tensor
        Normalized tensor where each sample has mean 0 and std 1 across features.
    """

    X_comp = X / X.sum(dim=-1, keepdim=True)
...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.cosine_loss_fn = nn.CosineEmbeddingLoss(margin=0.0)
    ...
    def configure_ddp(self):
        ...
        random.seed(self.config.random_seed + seed_offset)
        np.random.seed(self.config.np_seed + seed_offset)
        torch.manual_seed(self.config.torch_seed + seed_offset)
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.allow_tf32 = True

    ## Add this to extract first layer embedding
    def get_first_layer_embeddings(self, X, d=None):
        return self.col_embedder._train_forward(X, d=d, return_first=True)
    ...
    def run_micro_batch(self, micro_batch, micro_batch_idx, num_micro_batches):
        ...
        with self.amp_ctx:
            ## Clr on micro_x for Compositionality
            micro_X_cmp = closure(micro_X)

            ## Z-norm on micro_x for Compositionality
            # micro_X_cmp = z_normalize_samples(micro_X)

            # torch.save(micro_X_cmp, "/data/projects/deepintegromics/analyses/3.tabpfn/tab_icl/tabicl/src/tabicl_original/checkpoints/checkpoints_tabicl_similarity/micro_X_cmp.pt")
            # torch.save(micro_X, "/data/projects/deepintegromics/analyses/3.tabpfn/tab_icl/tabicl/src/tabicl_original/checkpoints/checkpoints_tabicl_similarity/micro_X.pt")
            # exit()

            # Choose model for DDP / single GPU
            model_to_use = self.model.module if self.ddp else self.model
            _, comp_row_emb = model_to_use._train_get_row_embeddings(micro_X_cmp, y_train, micro_d)
            pred, row_emb = model_to_use(micro_X, y_train, micro_d)  # forward pass

            # _ , comp_row_emb = self.model._train_get_row_embeddings(micro_X_cmp, y_train, micro_d)
            # pred, row_emb = self.model(micro_X, y_train, micro_d)  # (B, test_size, max_classes)
            pred = pred.flatten(end_dim=-2)
            true = y_test.long().flatten()

            # Cross-Entropy loss (classification model loss)
            ce_loss = F.cross_entropy(pred, true)

            ## Cosine similarity loss
            # alpha = 0.5
            z1 = comp_row_emb.reshape(-1, comp_row_emb.size(-1))  # CosineEmbeddingLoss expects (N, D)
            z2 = row_emb.reshape(-1, row_emb.size(-1))
            targets = torch.ones(z1.size(0), device=z1.device)  # Targets: +1 = similar
            cos_loss = self.cosine_loss_fn(z1, z2, targets)
        ...
        with torch.no_grad():
            ...
            micro_results["ce"] = ce_loss.item() / num_micro_batches
            micro_results["cos_loss"] = cos_loss.item() / num_micro_batches
    ...
    #     y_test = micro_y[:, train_size:]
    #
    #     # DDP sync only on final microbatch
    #     if self.ddp:
    #         self.model.require_backward_grad_sync = micro_batch_idx == num_micro_batches - 1
    #
    #     with self.amp_ctx:
    #         # Compositionality transform
    #         micro_X_cmp = clr_transform(micro_X, axis=-1)
    #
    #         # Choose model (DDP or direct)
    #         model_to_use = self.model.module if self.ddp else self.model
    #
    #         # Compute FIRST-LAYER embeddings for compositionality loss
    #         # Use unmasked input (d=None) for first-layer embeddings
    #         feat_emb_original = model_to_use.col_embedder._train_forward(
    #             micro_X, d=None, train_size=train_size, return_first=True
    #         )
    #         feat_emb_cmp = model_to_use.col_embedder._train_forward(
    #             micro_X_cmp, d=None, train_size=train_size, return_first=True
    #         )
    #
    #         # Flatten for cosine or MSE
    #         f1 = feat_emb_original.reshape(-1, feat_emb_original.size(-1))
    #         f2 = feat_emb_cmp.reshape(-1, feat_emb_cmp.size(-1))
    #
    #         targets = torch.ones(f1.size(0), device=f1.device)
    #         comp_loss = self.cosine_loss_fn(f1, f2, targets)
    #         # or: comp_loss = F.mse_loss(f1, f2)
    #
    #         #  Full forward for prediction loss
    #         pred, row_emb = model_to_use(micro_X, y_train, micro_d)
    #         pred = pred.flatten(end_dim=-2)
    #         true = y_test.long().flatten()
    #
    #         ce_loss = F.cross_entropy(pred, true)
    ...
    #         # Total loss
    #         alpha = 0.0
    #         loss = ce_loss * (1 - alpha) + comp_loss * alpha
    #
    #     # Gradient accumulation
    #     scaled_loss = loss / num_micro_batches
    #     self.scaler.scale(scaled_loss).backward()
    ...
    #             "ce": ce_loss.item() * (1 - alpha) / num_micro_batches,
    #             "cos_loss": comp_loss.item() * alpha / num_micro_batches,
