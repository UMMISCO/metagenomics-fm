...
class RowInteraction(nn.Module):
    ...
    def _aggregate_embeddings(self, embeddings: Tensor, key_mask: Optional[Tensor] = None) -> Tensor:
        ...
        cls_outputs = self.out_ln(outputs[..., : self.num_cls, :])

        # emb_outputs = outputs[..., self.num_cls :, :]  # (B, T, H*E) To UNCOMMENT

        # return cls_outputs.flatten(-2)  # (B, T, C*E)
        #return cls_outputs.flatten(-2), emb_outputs.flatten(-2) To UNCOMMENT
    ...
    def _train_forward(self, embeddings: Tensor, d: Optional[Tensor] = None) -> Tensor:
        ...
        representations, emb_representations  = self._aggregate_embeddings(embeddings, key_mask)  # (B, T, C*E)

        # return representations  # (B, T, C*E)
        return representations, emb_representations
