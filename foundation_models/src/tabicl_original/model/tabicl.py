...
class TabICL(nn.Module):
    ...
    def _train_get_row_embeddings(self, X: Tensor, y_train: Tensor, d: Optional[Tensor] = None, embed_with_test: bool = False
    ) -> Tensor:
        B, T, H = X.shape
        train_size = y_train.shape[1]
        assert train_size <= T, "Number of training samples exceeds total samples"
        ...
        # Column-wise embedding -> Row-wise interaction
        cls, embeddings = self.row_interactor(
            self.col_embedder(X, d=d, train_size=None if embed_with_test else train_size), d=d
        )
    ...
    def _train_forward(
        ...
        # Column-wise embedding -> Row-wise interaction
        representations, embeddings = self._train_get_row_embeddings(X, y_train, d)
    ...
    def _inference_forward(
        ...
        # Column-wise embedding -> Row-wise interaction
        ## i put , _  in order to unpack the tuple
        #representations, _ = self.row_interactor( To UNCOMMENT
        representations = self.row_interactor(
            self.col_embedder(
                X,
