...
class ColEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.zero_embedding = nn.Parameter(torch.randn(self.embed_dim)) ## We add this to embed zeros with special embedding vectors
    ...
    def _compute_embeddings(self, features: Tensor, train_size: Optional[int] = None) -> Tensor:
        ...
        # # detect real zeros (not -100)
        # zero_mask = (features == 0).squeeze(-1)  # (..., T)
        # pad_mask = (features == -100).squeeze(-1)  # (..., T)
        # apply_mask = zero_mask & (~pad_mask)  # only true for REAL zeros
        ...
        # # Overwrite embeddings where x == 0
        # # Prepare zero embedding with correct shape
        # # embeddings.dim() is always (..., T, E) → at least 3D
        # # we broadcast across all leading dims
        # embed_shape = [1] * (embeddings.dim() - 1) + [self.embed_dim]
        # zero_embed = self.zero_embedding.view(*embed_shape)
        # # Expand mask to match embedding dims
        # mask = apply_mask.unsqueeze(-1)  # (..., T, 1)
        #
        # embeddings = torch.where(mask, zero_embed, embeddings)

        return embeddings

    # def _compute_embeddings(self, features, train_size=None, return_first=False):
    #     # 1. First linear layer
    #     first_emb = self.in_linear(features)  # (..., T, E)
    #
    #     if return_first:
    #         return first_emb  # EARLY RETURN
    #
    #     # 2. Usual column transformer pipeline
    #     src = self.tf_col(first_emb, train_size)
    ...
    # def _train_forward(self, X, d=None, train_size=None, return_first=False):
    #     """
    #     Forward pass during training.
    #
    #     If return_first=True and d=None:
    #         → return ONLY the first-layer embeddings (before column & row transformers).
    #
    #     If return_first=True and d!=None:
    #         → NOT supported (masked path should not be used for α-loss).
    #
    #     Otherwise:
    #         → return the full embeddings through the entire pipeline.
    #     """
    #
    #     # 1. Pad CLS tokens if the model reserves any
    #     if self.reserve_cls_tokens > 0:
    ...
    #     device = X.device
    #
    #     #  UNMASKED PATH (NORMAL)
    #     if d is None:
    #
    #         # (B, T, H) → (B, H, T, 1)
    #         features = X.transpose(1, 2).unsqueeze(-1)
    #
    #         # ===== RETURN ONLY FIRST-LAYER EMBEDDINGS =====
    #         if return_first:
    #             first = self._compute_embeddings(
    #                 features, train_size, return_first=True
    #             )  # (B, H, T, E)
    #             return first.transpose(1, 2)  # (B, T, H, E)
    #
    #         # ===== FULL PIPELINE =====
    #         embeddings = self._compute_embeddings(features, train_size)
    #         return embeddings.transpose(1, 2)  # (B, T, H, E)
    #
    #     # MASKED PATH (ORIGINAL)
    #     else:
    #
    #         # IMPORTANT: return_first is NOT supported with masking
    #         if return_first:
    #             raise RuntimeError(
    #                 "return_first=True is only supported when d=None (unmasked path)."
    #             )
    #
    #         if self.reserve_cls_tokens > 0:
    #             d = d + self.reserve_cls_tokens
    ...
    #         B, HC, T = X.shape
    #
    #         # Build original 2D column mask
    #         indices = torch.arange(HC, device=device).unsqueeze(0).expand(B, HC)
    #         mask = indices < d.unsqueeze(1)  # (B, H+C)
    #
    #         # Extract valid feature rows
    #         features = X[mask].unsqueeze(-1)  # (N_valid, T, 1)
    #
    #         # Compute embeddings only for valid features
