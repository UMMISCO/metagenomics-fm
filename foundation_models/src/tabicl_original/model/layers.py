...
class OneHotAndLinear(nn.Linear):
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        # Replace -100 with 0 temporarily (to avoid one_hot error)
        safe_src = src.masked_fill(mask, 0)

        # One-hot encode
        one_hot = F.one_hot(safe_src.long(), num_classes=self.num_classes).to(src.dtype)

        # Zero out the entire one-hot vector at masked positions
        one_hot[mask] = 0
