...
# pasolli = abundance_cirrhosis - -stagediscovery
...
X, y = open_pasolli('abundance_WT2D')
X, y = open_and_filter(X, 0.0, 0.0)

import torch
torch.save(X, "/data/projects/deepintegromics/analyses/3.tabpfn/deepmicro/DeepMicro/X.npy")
torch.save(y, "/data/projects/deepintegromics/analyses/3.tabpfn/deepmicro/DeepMicro/y.npy")
...
class PreprocessingPipeline(TransformerMixin, BaseEstimator):
    ...
    def __init__(
        self, normalization_method: str = "power", preprocess: bool = True, outlier_threshold: float = 4.0, random_state: Optional[int] = None
    ):
    ...
    def transform(self, X):
        ...
        if hasattr(self, "standard_scaler_"):
            X = self.standard_scaler_.transform(X)
        else:
            print("Skipping scaling")

        # Step 2: Normalization
        if getattr(self, "normalizer_", None) is not None:
            print(f"Applying normalization (method={self.normalization_method})")
            try:
            ...
            print(f"Skipping normalization (method={self.normalization_method})")

        # Step 3: Outlier removal
        if hasattr(self, "outlier_remover_"):
...
# Your preprocessing classes must already be imported here
# from preprocessing import PreprocessingPipeline   # ← example
# and X, y already loaded:
# X, y = open_pasolli('abundance_WT2D')
# X, y = open_and_filter(X, 0.0, 0.0)
...
p = (
    ggplot(df_plot, aes("tSNE1", "tSNE2", color="label"))
    + geom_point(size=3, alpha=0.7)
    + ggtitle("t-SNE of Preprocessed Data")
    + theme_minimal()
)
