...
from tabicl_original.prior.prior_config import DEFAULT_FIXED_HP, DEFAULT_SAMPLED_HP
...
class LoadPriorDataset(IterableDataset):
    ...
    def __iter__(self):
        # logger.debug(f"__iter__ called in PID {os.getpid()}")
        return self
    def _load_batch_file(self):
        ...
        if X.is_nested:
            ...
            ### Set max_len if you don't want max_len = max number of features of the batch
            # TODO: remove hard coded max_len
...
class SavePriorDataset:
    ...
    def save_batch_sparse(self, batch_idx, X, y, d, seq_lens, train_sizes, assignments):
        ...
        torch.save(
            {"X": X, "y": y, "d": d, "seq_lens": seq_lens, "train_sizes": train_sizes, "batch_size": B, "feature_assignments": assignments,},
            temp_file,
        )
    ...
    def run(self):
        ...
        for batch_idx in tqdm(
            ...
            X, y, d, seq_lens, train_sizes, assignments = self.prior.get_batch()
            ...
            self.save_batch_sparse(batch_idx, X, y, d, seq_lens, train_sizes, assignments)
