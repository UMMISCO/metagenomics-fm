...
class SCMPrior(Prior):
    ...
    #
    #     while True:
    #         # X, y, assignments = prior_cls(**params)()
    ...
    #         assignments = assignments[:d.item()]
    #
    #         if (d > 0).all() and self.sanity_check(X, y, params["train_size"]):
    #             return X.squeeze(0), y.squeeze(0), d.squeeze(0), assignments
    @torch.no_grad()
    def generate_dataset(self, params: Dict[str, Any]) -> Tuple[Tensor, Tensor, Tensor]:
        """
        Generates a single valid dataset based on the provided parameters.
    ...
    def  get_batch(self, batch_size: Optional[int] = None) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        for gp_idx in range(num_gps):
            ...
            for subgp_idx in range(num_subgps_in_gp):
                ...
                # print('vediiiiiiiii',subgp_sampled_hp)  #GIULIA
        ...
        # X_list, y_list, d_list, assignment_list = zip(*results)
        ...
        #
        # if rank == 0:
        #     save_dir = "tabicl/checkpoints/checkpoints_try1"
        #     os.makedirs(save_dir, exist_ok=True)
        #
        #     save_path = os.path.join(save_dir, f"batch_{self.batch_counter:05d}.pt")
        #
        #     torch.save(
        #         {
        #             "X": X.cpu(),
        ...
        #     )
        #
        #     print(f"[INFO] Saved batch to {save_path}")
        #
        #     self.batch_counter += 1
        # # ----------------------------------

        # return X, y, d, seq_lens, train_sizes, assignment_list
    ...
    def get_prior(self) -> str:
        ...
        if self.prior_type == "mix_scm":
            return np.random.choice(["mlp_scm", "tree_scm"], p=self.fixed_hp.get("mix_probs", ))
        else:
...
class PriorDataset(IterableDataset):
    ...
    def get_batch(self, batch_size: Optional[int] = None) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        # X, y, d, seq_lens, train_sizes, assignments = self.prior.get_batch(batch_size)
        # return X, y, d, seq_lens, train_sizes, assignments
