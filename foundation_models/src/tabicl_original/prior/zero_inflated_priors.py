...
class ZeroInflatedDataGenerator:
    ...
    #     causes = torch.rand((self.n_samples, 1, self.num_causes), device=self.device).float()
    #     return causes

    def generate_ziu_data(self):
        z = torch.rand((self.n_samples, self.num_causes), device=self.device).float()
        bernoulli_samples = self.generate_bernoulli_data(z.shape)
