...
def eval_criterion(criterion, targets, output, device, n_out):
    ...
    if isinstance(criterion, nn.GaussianNLLLoss):
        ...
        # print(int(targets.max()) + 1, n_out)
        # print(output.reshape(-1, n_out).shape)
        # print(output.reshape(-1, n_out)[:, :int(targets.max()) + 1].shape)
...
def train_epoch(model, aggregate_k_gradients, using_dist, scaler, dl, device, optimizer, criterion, n_out):
    ...
    assert len(dl) % aggregate_k_gradients == 0, f'Please set the number of steps per epoch s.t. `aggregate_k_gradients` divides it., {aggregate_k_gradients}, {len(dl)}'
...
def train(rank, using_dist, dl, model, criterion, optimizer_state=None, scheduler=None,
    ...
    elif (type(device) is int) or (device == "cuda"):
    ...
    try:
        for epoch in range(start_epoch, epochs + 1):
            ...
            total_loss = new_loss
            loss_history.append(total_loss)  # Store loss of current epoch
