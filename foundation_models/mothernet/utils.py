...
def get_model_string(config):
    model_string = (f"{config.experiment_number}")
    ...
    def save_callback(model, optimizer, scheduler, epoch):
        ...
        try:
            if (epoch == "on_exit") or epoch % save_every == 0:
                ...
                config_copy['epoch_in_training'] = epoch
                config_copy['learning_rates'] = model.learning_rates
                config_copy['losses'] = model.losses
                config_copy['wallclock_times'] = model.wallclock_times

                save_model(model, optimizer, scheduler, base_path, file_name, config_copy)
