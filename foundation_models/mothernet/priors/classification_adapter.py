...
from mothernet.utils import (nan_handling_missing_for_a_reason_value, nan_handling_missing_for_no_reason_value,
                             nan_handling_missing_for_unknown_reason_value, normalize_by_used_features_f,
                             normalize_data,
                             remove_outliers)
...
from mothernet.priors.mlp import MLPPrior, normalize_rows_to_value
...
class MulticlassSteps:
    ...
    def __call__(self, x):
        ...
        class_boundary_indices = torch.randint(0, x.shape[0], ((self.num_classes - 1) * (self.num_steps - 1) + 1,),
                                               device=x.device)
        ...
        class_assignments = torch.randint(0, self.num_classes, (step_assignments.max() + 1, x.shape[1]),
                                          device=x.device)
...
class ClassificationAdapter:
    ...
    def __call__(self, batch_size, n_samples, num_features, device, epoch=None, single_eval_pos=None):
        ...
        x += 1  ## Add it for zero-inflation

        assert x.shape[2] == self.h['num_features_used']

        if (self.h['nan_prob_no_reason'] + self.h['nan_prob_a_reason'] + self.h['nan_prob_unknown_reason'] > 0
                and random.random() > 0.5):  # Only one out of two datasets should have nans
        ...
        if self.h['enable_normalization']:
        ...
        if self.h['normalize_by_rows']:
            x = torch.stack([normalize_rows_to_value(x[:, i, :]) for i in range(x.size(1))], dim=1)   # Compositionality
        ...
        x = torch.cat(
            [x, torch.zeros((x.shape[0], x.shape[1], num_features - self.h['num_features_used']),
                            device=device)],
            -1)  ### Here the number of features is augmented to the maximum number, the features added are 0
        ...
        for b in range(y.shape[1]):
            ...
            y[valid_labels, b] = (y[valid_labels, b] > y[valid_labels, b].unique().unsqueeze(1)).sum(axis=0).unsqueeze(
                0).float()
...
class ClassificationAdapterPrior:
    def __init__(self, config):
        if isinstance(config, dict):
            ...
            self.base_prior = MLPPrior(config['mlp'])
        else:
            self.config = config.classification
            self.base_prior = MLPPrior(config.mlp)
    ...
    def get_batch(self, batch_size, n_samples, num_features, device, epoch=None, single_eval_pos=None):
        with torch.no_grad():
            args = {'device': device, 'n_samples': n_samples, 'num_features': num_features,
                    'epoch': epoch, 'single_eval_pos': single_eval_pos}
