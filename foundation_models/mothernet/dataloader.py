...
class PriorDataLoader(DataLoader):
    ...
    def gbm(self, epoch=None):
        ...
        batch = self.prior.get_batch(batch_size=self.batch_size, n_samples=self.n_samples,
...
def get_dataloader(prior_config, dataloader_config, device):
    ...
    mlp_flexible = ClassificationAdapterPrior(config=prior_config)
    if prior_type == 'prior_bag':
        # Prior bag combines priors
        prior = BagPrior(base_priors={'mlp': mlp_flexible},
                         prior_weights={'mlp': 1})
    ...
    return PriorDataLoader(prior=prior, num_steps=dataloader_config['num_steps'],
